...
int main()
{
    int at[20],bt[20],pr[20],ct[20],rt[20],tat[20],flag[20];
    int n,i,sum_bt,curt,priority,exec;//exec is index of process to be executed;
    ...
    cout<<"\nEnter arrival and burst times and priority of process ";
    for(i=0;i<n;i++)
    {
        cin>>at[i]>>bt[i]>>pr[i];
        sum_bt=sum_bt+bt[i];
    ...
    while(curt!=sum_bt)
    {
        priority=100;         //Assumed burst time for algorithm satisfaction for finding min burst time
        for(i=0;i<n;i++)
        {
    ...
    for(i=0;i<n;i++)
    {
        cout<<i+1<<"\t"<<at[i]<<"\t"<<bt[i]<<"\t"<<pr[i]<<"\t"<<ct[i]<<"\t"<<tat[i]<<"\t"<<rt[i]<<"\n";
    }
